...
def push_to_model_registry(
    ...
    if prod_flag:
        ...
        def _generate_artifact_name(artifact_file_name, version=""):
            artifact_name, artifact_extension = path.splitext(
                path.basename(artifact_file_name)
            )
            artifact_version_file_name = (
                f"{artifact_name}-{version}{artifact_extension}"
            )
        ...
        def _do_upload(s3_client, model_path, object_name, s3_bucket_name):
            ...
            try:
                ...
                print(
                    f"S3 upload to bucket {s3_bucket_name} at {s3_endpoint_url} failed: {e}"
                )
                raise
        ...
        _do_upload(
            s3_client,
            label_encoder.path,
            label_encoder_artifact_s3_path,
            s3_bucket_name,
        )

    environ["KF_PIPELINES_SA_TOKEN_PATH"] = (
    ...
    def _register_model(author_name, server_address, model_object_prefix, version):
        registry = ModelRegistry(
            server_address=server_address, port=443, author=author_name, is_secure=False
        )
        ...
        rm = registry.register_model(
            ...
            (
                "to-be-updated"
                if prod_flag
                else f"s3://{s3_endpoint_url.split('https://')[-1]}{model_artifact_s3_path}"
            ),
