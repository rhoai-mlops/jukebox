...
def evaluate_keras_model_performance(
    ...
    environ["KF_PIPELINES_SA_TOKEN_PATH"] = (
        "/var/run/secrets/kubernetes.io/serviceaccount/token"  # Hotfix to access the endpoint
    )
    ...
    try:
        ...
        for v in registry.get_model_versions(model_name).order_by_id().descending():
            if not previous_model_properties:
                previous_model_properties = (
                    registry.get_model_versions(model_name)
                    .order_by_id()
                    .descending()
                    .next_item()
                    .custom_properties
    ...
    metrics.log_metric(
        "Prev Model Accuracy", float(previous_model_properties["accuracy"])
    )
...
@component(
    base_image="tensorflow/tensorflow",
    packages_to_install=["onnxruntime", "pandas", "scikit-learn"],
)
def validate_onnx_model(
    ...
    onnx_session = rt.InferenceSession(
        onnx_model.path, providers=rt.get_available_providers()
    )
    ...
    onnx_pred = onnx_session.run(
        [onnx_output_name],
        {onnx_input_name: scaler_.transform(X_test.values).astype(np.float32)},
    )
